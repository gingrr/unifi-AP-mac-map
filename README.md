...
## Portability / other platforms

This script is written and tested specifically for **UniFi APs running BusyBox-based firmware**, where:

- `iwconfig` shows entries like:  
  `Mode:Master  Frequency:5.745 GHz  Access Point: F0:9F:C2:FF:10:32`  
- ESSIDs (`ESSID:"..."`) and BSSIDs (`Access Point: ...`) appear in the same textual block.  

In theory, the approach will also work on **other vendors that:**

- Run a **BusyBox-style embedded Linux** userland, and  
- Expose wireless interfaces and BSSIDs via `iwconfig` (or compatible tools), and  
- Allow SSH access with shell commands.

Examples of platforms where a lightly adapted version of this script *might* work:

- Older / classic **Ubiquiti** gear beyond UniFi (e.g. some airMAX / EdgeMAX devices with Wi-Fi)  
- **OpenWrt-based** APs and routers using BusyBox + `iwconfig`  
- Some older **TP-Link / Omada** APs  
- Some **Linksys WRT** and similar devices  
- OEM / white-label APs that ship BusyBox firmware and expose `iwconfig`  

However:

- Interface names (`ath0`, `ath1`, `wlan0`, `wifi0`, `vap0`, etc.) and `iwconfig` output formats differ across vendors.  
- Some newer platforms (including some UniFi WiFi 6 / 7 or cloud-managed APs from Cisco/Meraki/Aruba/Ruckus) may **not** expose `iwconfig` or a standard shell at all.  
- On those systems, MAC/BSSID information is usually only available via the vendor’s own CLI/API, not Linux tools.

So while the **technique** (SSH → iwconfig → parse ESSID/Frequency/Access Point) is portable, this script is intentionally scoped to UniFi-like BusyBox environments. Porting it to other vendors usually means:

- Updating the detection of wireless interfaces  
- Adjusting the parsing regex for that platform’s `iwconfig` (or `iw`) output  
- Possibly replacing `iwconfig` entirely with vendor-specific CLI commands  
